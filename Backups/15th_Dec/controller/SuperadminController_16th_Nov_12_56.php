 ...
 */
	...
	function beforeFilter()
	{
		 ...
		 $this->Auth->allow('register','login', 'product_attribute_change', 'change_slider_img_order', 'category_tree_one');  
	...
	function category_tree_one($catid, $selected_id, $sele){
		...
		foreach($category_data as $data)
		{
			 ...
			 {
				 if($sele == $data['id'])
				 echo '<option selected="selected" value="'.$data['parentid'].'">'.$data['catname'].'</option>';
				 else
				 echo '<option value="'.$data['parentid'].'">'.$data['catname'].'</option>';
			 }
			 else
			 echo '<option value="'.$data['parentid'].'">'.$data['catname'].'</option>';
	...
	public function category_edit($id) 
	{
		...
		$category_data = $this->Category->find('first', array('conditions'=>array('Category.id'=>$id), 'conditions' => array('Category.del_status' => 0)));
